...
#define MM_REG_TEX_OFFSET 0
#include <material_maps.hlsl>

#define LGH_REG_CBUF_OFFSET 2
#define LGH_REG_SB_OFFSET MM_REG_TEX_LEN
#include <lights.hlsl>
...
float4 main(Vertex world) : SV_Target {
    ...
    [branch] if (mtl_config & MTL_CONFIG_HAS_TEXTURE) ambient_tex = TextureMap.Sample(SamplerTexture, tex_uv).rgb;
    [branch] if (mtl_config & MTL_CONFIG_HAS_DIFFUSE_MAP) diffuse_tex = DiffuseMap.Sample(SamplerDiffuse, tex_uv).rgb;
    ...
    [flatten] if (mtl_config & MTL_CONFIG_HAS_FILTER) color *= mtl_clr_filter;
