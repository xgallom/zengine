...
pub fn RadixTree(comptime V: type) type {
    return struct {
        ...
        const Pool = std.heap.MemoryPool(PoolItem);
        const PoolItem = union {
            node: Node,
            edge: Edge,
        };

        pub const Edges = std.SinglyLinkedList(struct {
            label: []const u8,
            target: *Node,
        });
        pub const Edge = Edges.Node;
        ...
        pub const Node = struct {
            ...
            fn clearEdges(node: *Node, self: *Self) usize {
                ...
                while (node.edges.popFirst()) |edge_node| {
                    count += edge_node.data.target.deinit(self);
                    self.destroyEdge(edge_node);
                }
                return count;
            }
        ...
        pub fn init(allocator: std.mem.Allocator, preheat: usize) !Self {
            ...
            const pool_item: *PoolItem = try result.pool.create();
            pool_item.node = .{};
            result.root = &pool_item.node;
            return result;
        }
        ...
        pub fn insert(self: *Self, label: []const u8, value: Value) !void {
            ...
            log.debug("insert [{s}]: {any}", .{ label, value });
            while (true) {
        ...
        pub fn search(self: *const Self, label: []const u8, search_type: SearchType) ?Value {
        ...
        pub fn searchPrefix(self: *const Self, label: []const u8) ?Value {
            var walk = self.root;
            var needle = label;
            ...
            if (walk.isLeaf()) return walk.value;
            const edge = walk.edges.first.?.data;
            if (edge.label.len == 0) return edge.target.value;
            return null;
        }

        /// Searches tree and returns value matching searched label exactly
        pub fn searchExact(self: *const Self, label: []const u8) ?Value {
            var walk = self.root;
            var needle = label;
        ...
        pub fn searchSuffix(self: *const Self, label: []const u8) ?Value {
            var walk = self.root;
            var needle = label;
            ...
            log.debug("found edge", .{});
            while (!walk.isLeaf()) : (walk = walk.edges.first.?.data.target) {}
            return walk.value;
        }

        fn splitEdge(self: *Self, edge: *Edge.Data, base_label: []const u8, node_label: []const u8, new_label: []const u8, new_value: Value) !void {
        ...
        fn orderedInsert(head: *?*Edge, edge: *Edge) void {
            if (head.* == null or order(head.*.?, edge).compare(.gte)) {
                edge.next = head.*;
                head.* = edge;
                return;
            }
            ...
            var current = head.*.?;
            while (current.next != null and order(current.next.?, edge).compare(.lt)) : (current = current.next.?) {}
        ...
        fn order(lhs: *Edge, rhs: *Edge) std.math.Order {
            return std.mem.order(u8, lhs.data.label, rhs.data.label);
        }
        ...
        fn destroyNode(self: *Self, node: *Node) void {
            const offset = @intFromPtr(node);
            const pool_item: *PoolItem = @ptrFromInt(offset - @offsetOf(PoolItem, "node"));
            self.pool.destroy(pool_item);
        }

        fn destroyEdge(self: *Self, edge: *Edge) void {
            const offset = @intFromPtr(edge);
            const pool_item: *PoolItem = @ptrFromInt(offset - @offsetOf(PoolItem, "edge"));
            self.pool.destroy(pool_item);
        }
